...
namespace VersionManager\Core\Extension\Drivers\Yaml;
...
class Base extends AbstractExtensionDriver
{
    ...
    protected function installDebianDependencies()
    {
        ...
        if ($returnCode !== 0) {
            throw new \Exception("安装 YAML 依赖失败: " . implode("\n", $output));
        }
    }
    ...
    {
        $command = 'yum install -y libyaml-devel';
    ...
    {
        $command = 'apk add --no-cache yaml-dev';
