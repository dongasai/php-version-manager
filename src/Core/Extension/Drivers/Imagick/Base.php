...
namespace VersionManager\Core\Extension\Drivers\Imagick;
...
class Base extends AbstractExtensionDriver
{
    ...
    {
        $command = 'apt-get update && apt-get install -y libmagickwand-dev';
        ...
        if ($returnCode !== 0) {
            throw new \Exception("安装 Imagick 依赖失败: " . implode("\n", $output));
        }
    }
    ...
    {
        $command = 'yum install -y ImageMagick-devel';
    ...
    {
        $command = 'apk add --no-cache imagemagick-dev';
