...
namespace VersionManager\Core\Extension\Drivers\Zstd;
...
class Base extends AbstractExtensionDriver
{
    ...
    {
        $command = 'apt-get update && apt-get install -y libzstd-dev';
        ...
        if ($returnCode !== 0) {
            throw new \Exception("安装 Zstandard 依赖失败: " . implode("\n", $output));
        }
    }
    ...
    {
        $command = 'yum install -y libzstd-devel';
    ...
    {
        $command = 'apk add --no-cache zstd-dev';
