...
namespace VersionManager\Core\Extension\Drivers\Bz2;
...
class Base extends AbstractExtensionDriver
{
    ...
    public function install($phpVersion, array $options = [])
    {
        ...
        if (empty($output)) {
            ...
            throw new \Exception("当前 PHP 版本不支持 BZip2 扩展，需要重新编译 PHP");
        }
    ...
    {
        $command = 'apt-get update && apt-get install -y libbz2-dev';
        ...
        if ($returnCode !== 0) {
            throw new \Exception("安装 BZip2 依赖失败: " . implode("\n", $output));
        }
    }
    ...
    {
        $command = 'yum install -y bzip2-devel';
    ...
    {
        $command = 'apk add --no-cache bzip2-dev';
