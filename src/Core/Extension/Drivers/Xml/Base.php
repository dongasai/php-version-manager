...
namespace VersionManager\Core\Extension\Drivers\Xml;
...
class Base extends AbstractExtensionDriver
{
    ...
    public function install($phpVersion, array $options = [])
    {
        ...
        if (empty($output)) {
            ...
            throw new \Exception("当前 PHP 版本不支持 XML 扩展，需要重新编译 PHP");
        }
    ...
    protected function installDebianDependencies()
    {
        ...
        try {
            $method->invoke($installer, ['libxml2-dev']);
        } catch (\Exception $e) {
            throw new \Exception("安装 XML 依赖失败: " . $e->getMessage());
        }
    }
    ...
    {
        $command = 'yum install -y libxml2-devel';
        ...
        if ($returnCode !== 0) {
            throw new \Exception("安装 XML 依赖失败: " . implode("\n", $output));
        }
    }
    ...
    {
        $command = 'apk add --no-cache libxml2-dev';
