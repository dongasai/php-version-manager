...
namespace VersionManager\Core\Extension\Drivers\Imap;
...
class Base extends AbstractExtensionDriver
{
    ...
    public function install($phpVersion, array $options = [])
    {
        ...
        if (empty($output)) {
            ...
            throw new \Exception("当前 PHP 版本不支持 IMAP 扩展，需要重新编译 PHP");
        }
    ...
    {
        $command = 'apt-get update && apt-get install -y libc-client-dev libkrb5-dev';
        ...
        if ($returnCode !== 0) {
            throw new \Exception("安装 IMAP 依赖失败: " . implode("\n", $output));
        }
    }
    ...
    {
        $command = 'yum install -y libc-client-devel krb5-devel';
    ...
    {
        $command = 'apk add --no-cache imap-dev krb5-dev';
