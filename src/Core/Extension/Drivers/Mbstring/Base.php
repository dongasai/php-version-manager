...
namespace VersionManager\Core\Extension\Drivers\Mbstring;
...
class Base extends AbstractExtensionDriver
{
    ...
    public function install($phpVersion, array $options = [])
    {
        ...
        if (empty($output)) {
            ...
            throw new \Exception("当前 PHP 版本不支持 Mbstring 扩展，需要重新编译 PHP");
        }
    ...
    {
        $command = 'apt-get update && apt-get install -y libonig-dev';
        ...
        if ($returnCode !== 0) {
            throw new \Exception("安装 Mbstring 依赖失败: " . implode("\n", $output));
        }
    }
    ...
    {
        $command = 'yum install -y oniguruma-devel';
    ...
    {
        $command = 'apk add --no-cache oniguruma-dev';
