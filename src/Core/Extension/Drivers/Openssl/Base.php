...
namespace VersionManager\Core\Extension\Drivers\Openssl;
...
class Base extends AbstractExtensionDriver
{
    ...
    public function install($phpVersion, array $options = [])
    {
        ...
        if (empty($output)) {
            ...
            throw new \Exception("当前 PHP 版本不支持 OpenSSL 扩展，需要重新编译 PHP");
        }
    ...
    protected function installDebianDependencies()
    {
        ...
        if ($returnCode !== 0) {
            throw new \Exception("安装 OpenSSL 依赖失败: " . implode("\n", $output));
        }
    }
    ...
    {
        $command = 'apk add --no-cache openssl-dev';
