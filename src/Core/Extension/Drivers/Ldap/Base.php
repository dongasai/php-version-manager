...
namespace VersionManager\Core\Extension\Drivers\Ldap;
...
class Base extends AbstractExtensionDriver
{
    ...
    public function install($phpVersion, array $options = [])
    {
        ...
        if (empty($output)) {
            ...
            throw new \Exception("当前 PHP 版本不支持 LDAP 扩展，需要重新编译 PHP");
        }
    ...
    {
        $command = 'apt-get update && apt-get install -y libldap2-dev';
        ...
        if ($returnCode !== 0) {
            throw new \Exception("安装 LDAP 依赖失败: " . implode("\n", $output));
        }
    }
    ...
    {
        $command = 'apk add --no-cache openldap-dev';
